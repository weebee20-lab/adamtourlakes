  ...
  return (
    <header className="sticky top-0 z-40 border-0 bg-bg/40 shadow-none backdrop-blur-xl">
      <div className="mx-auto flex h-12 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <nav className="site-desk-nav hidden min-w-0 flex-1 items-center justify-end gap-0.5 xl:flex" aria-label="Primary">
          {NAV.map((item) => {
        ...
        <div className="flex items-center gap-2">
          <Button asChild size="sm" className="site-desk-quote hidden xl:inline-flex">
            <Link to="/contact">
              Get a quote
            </Link>
          </Button>
          <button
            type="button"
            className="site-menu-btn inline-flex size-10 items-center justify-center rounded-md text-fg xl:hidden"
            aria-expanded={open}
      ...
      {open ? (
        <div
          id="mobile-nav"
          className="site-mobile-nav border-t border-border bg-bg px-4 py-4 xl:hidden"
        >
